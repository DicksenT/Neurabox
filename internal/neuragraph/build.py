...
def build_from_json(extraction: dict, *, directed: bool = False, root: str | Path | None = None) -> nx.Graph:
    ...
    for node in extraction.get("nodes", []):
        ...
        if "source" in node and "source_file" not in node:
            ...
            print(
                f"[neuragraph] WARNING: node '{node_id}' uses field 'source' instead of "
        ...
        # neuragraph versions that didn't always populate file_type) don't
    ...
    if real_errors:
        print(f"[neuragraph] Extraction warning ({len(real_errors)} issues): {real_errors[0]}", file=sys.stderr)
...
def build(
    ...
    *,
    ...
    """
    from neuragraph.dedup import deduplicate_entities
...
def deduplicate_by_label(nodes: list[dict], edges: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    print(f"[neuragraph] Deduplicated {len(remap)} duplicate node(s) by label.", file=sys.stderr)
...
def build_merge(
    ...
    graph_path: str | Path = "neuragraph-out/graph.json",
    ...
    *,
    ...
    if graph_path.exists():
        ...
        from neuragraph.security import check_graph_file_size_cap
    ...
    if prune_sources:
        ...
        if n_nodes:
            print(
                f"[neuragraph] Pruned {n_nodes} node(s) from {n_files} deleted source file(s).",
                file=sys.stderr,
            )
        ...
        if edges_to_remove:
            ...
            print(
                f"[neuragraph] Pruned {len(edges_to_remove)} edge(s) from deleted source file(s).",
                file=sys.stderr,
            )

        if not n_nodes and not edges_to_remove:
            print(
                f"[neuragraph] {n_files} source file(s) deleted since last run — "
    ...
    if graph_path.exists() and not dedup and not prune_sources:
        ...
        if new_n < existing_n:
            raise ValueError(
                f"neuragraph: build_merge would shrink graph from {existing_n} → {new_n} nodes. "
