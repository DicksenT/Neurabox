...
GOOGLE_WORKSPACE_EXTENSIONS = set()
def convert_google_workspace_file(*args, **kwargs): return None
def google_workspace_enabled(): return False
...
_MANIFEST_PATH = "nb-graph/manifest.json"
...
def xlsx_extract_structure(path: Path) -> dict:
    ...
    Returns a nodes/edges dict compatible with the neuragraph extract pipeline.
...
_SKIP_DIRS = {
    ...
    ".tox", ".eggs", "*.egg-info",
    "neuragraph-out",  # never treat own output as source input (#524)
    ...
    ".neuragraph",  # neuragraph's own extraction cache — never index self-generated data
...
def _parse_gitignore_line(raw: str) -> str:
    """Parse one raw line from a .neuragraphignore file per gitignore spec.
...
def _load_neuragraphignore(root: Path) -> list[tuple[Path, str]]:
    """Read .neuragraphignore files and return (anchor_dir, pattern) pairs.
    ...
    for d in dirs:
        # Prefer .neuragraphignore; fall back to .gitignore so projects that already
        # maintain a .gitignore get sensible defaults without duplicating it (#945).
        ignore_file = d / ".neuragraphignore"
...
def _is_ignored(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if the path should be ignored per .neuragraphignore patterns.
...
def _load_neuragraphinclude(root: Path) -> list[tuple[Path, str]]:
    """Read .neuragraphinclude allowlist patterns from root and ancestors.
    ...
    Uses the same VCS-root ceiling logic as _load_neuragraphignore.
    """
    root = root.resolve()
    ceiling = _find_vcs_root(root) or root
    ...
    patterns: list[tuple[Path, str]] = []
    for d in dirs:
        include_file = d / ".neuragraphinclude"
...
def _is_included(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if path matches any .neuragraphinclude allowlist pattern."""
    if not patterns:
        return False
...
def _could_contain_included_path(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if a directory may contain files matched by .neuragraphinclude."""
    if not patterns:
        return False
...
def detect(root: Path, *, follow_symlinks: bool | None = None, google_workspace: bool | None = None, extra_excludes: list[str] | None = None) -> dict:
    ...
    ignore_patterns = _load_neuragraphignore(root)
    # CLI --exclude patterns are anchored at the scan root and appended last
    # so they win over any .neuragraphignore/.gitignore rules (#947).
    if extra_excludes:
        for pat in extra_excludes:
            line = _parse_gitignore_line(pat)
            if line:
                ignore_patterns.append((root, line))
    include_patterns = _load_neuragraphinclude(root)

    # Always include neuragraph-out/memory/ - query results filed back into the graph
    memory_dir = root / "neuragraph-out" / "memory"
    scan_paths = [root]
    ...
    converted_dir = root / "neuragraph-out" / "converted"
        ...
        if ftype:
            if p.suffix.lower() in GOOGLE_WORKSPACE_EXTENSIONS:
                if not google_workspace:
                    skipped_sensitive.append(
                        ...
                        "or set neuragraph_GOOGLE_WORKSPACE=1]"
                    )
                    continue
                try:
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - pip install neuragraphy[office]]")
                continue
    ...
    return {
        ...
        "neuragraphignore_patterns": len(ignore_patterns),
...
def save_manifest(
    ...
    *,
    ...
    kind="ast"      — written by `neuragraph update` (AST-only rebuild). Stamps
                      ast_hash; preserves an existing semantic_hash only when
                      the file content is unchanged (mtime + hash match).
    kind="semantic" — written by `neuragraph extract` after semantic extraction.
...
def detect_incremental(
    ...
    *,
    ...
    kind="semantic" (default for extract): a file is "changed" when its
        ...
        semantic extraction pass. Use this for `neuragraph extract` so that
        files touched by `neuragraph update` (AST-only) are re-extracted
        semantically.
    kind="ast": a file is "changed" when its ast_hash is missing or its
        content has changed. Use this for `neuragraph update`.
