...
try:
    import neuragraph.detect
    print("neuragraph.detect imported OK")
...
def extract_mcp_config(path): return {"nodes": [], "edges": []}
def is_mcp_config_path(path): return False
...
def _safe_extract(extractor: Callable, path: Path) -> dict:
    try:
        ...
        if os.environ.get("neuragraph_DEBUG"):
            import traceback
...
def _pascal_resolve_unit(from_path: Path, unit_name: str) -> str:
    """Resolve a Pascal unit name to the neuragraph node ID of its source file.
...
def _check_tree_sitter_version() -> None:
    ...
    if LANGUAGE_VERSION < 14:
        ...
        raise RuntimeError(
            ...
            f"neuragraph requires tree-sitter >= 0.23.0 (Language API v2). "
...
def extract_bash(path: Path) -> dict:
    ...
    from neuragraph.security import sanitize_metadata  # module-level cached import
...
def _extract_parallel(
    ...
    if max_workers is None:
        # Honour neuragraph_MAX_WORKERS env override; otherwise scale to the
        ...
        env_raw = os.environ.get("neuragraph_MAX_WORKERS", "").strip()
        env_cap = None
        if env_raw:
            try:
                v = int(env_raw)
...
def extract(
    ...
    *,
    ...
    Args:
        ...
        cache_root: explicit root for neuragraph-out/cache/ (overrides the
            inferred common path prefix). Pass Path('.') when running on a
            subdirectory so the cache stays at ./neuragraph-out/cache/.
        ...
        max_workers: max subprocess count. Defaults to cpu_count (or the
            value of neuragraph_MAX_WORKERS if set), bounded by len(uncached_work).
    """
...
def collect_files(target: Path, *, follow_symlinks: bool = False, root: Path | None = None) -> list[Path]:
    ...
    from neuragraph.detect import _load_neuragraphignore, _is_ignored, _is_noise_dir
    ignore_root = root if root is not None else target
    patterns = _load_neuragraphignore(ignore_root)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m neuragraph.extract <file_or_dir> ...", file=sys.stderr)
        sys.exit(1)
