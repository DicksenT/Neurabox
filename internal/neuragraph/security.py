...
def _ssrf_guarded_socket():
    ...
    Not thread-safe, but neuragraph is a single-threaded CLI tool.
    """
...
def safe_fetch(url: str, max_bytes: int = _MAX_FETCH_BYTES, timeout: int = 30) -> bytes:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 neuragraph/1.0"})
...
def validate_graph_path(path: str | Path, base: Path | None = None) -> Path:
    ...
    *base* defaults to the `neuragraph-out` directory relative to CWD.
    Also requires the base directory to exist, so a caller cannot
    trick neuragraph into reading files before any graph has been built.
    ...
    if base is None:
        ...
        for candidate in [resolved_hint, *resolved_hint.parents]:
            if candidate.name == "neuragraph-out":
                base = candidate
                break
        if base is None:
            base = Path("neuragraph-out").resolve()

    base = base.resolve()
    if not base.exists():
        raise ValueError(
            f"Graph base directory does not exist: {base}. "
            "Run /neuragraph first to build the graph."
        )
    ...
    try:
        ...
        raise ValueError(
            ...
            "Only paths inside neuragraph-out/ are permitted."
        )
